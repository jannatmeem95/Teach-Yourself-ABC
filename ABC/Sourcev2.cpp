...
char *bodysound[14] = { "arm.wav", "ear.wav", "eye.wav", "nose.wav", "lips.wav", "neck.wav", "teeth.wav", "tongue.wav", "hand.wav", "leg.wav", "shoulder.wav", "knee.wav", "head.wav", "forehead.wav"};
...
char *whiwav[2] = { "whiteeth.wav", "milk.wav" };
char *blawav[2] = { "blackhair.wav", "crow.wav" };
...
char * pic_color[10][3] =
{
	...
	{ "carrot1.bmp", "orange2.bmp", "orange12.bmp" },
...
void page_no_9_parameters()   ///// alpha to pic
{
	...
	chouka = rand() % alphapiccount[AtoPPosi];
	pachka = rand() % alphapiccount[ekka];
	chokka = rand() % alphapiccount[dokka];
	shatka = rand() % alphapiccount[tekka];
...
void page_no_10_parameters()   ///// alpha to pic
{
	AtoPSelect = rand() % 26;

	//PlaySound( alpha_sound[AtoPSelect][ekka],NULL ,SND_FILENAME | SND_ASYNC);
	AtoPPosi = rand() % 4;
	ekka = rand() % alphapiccount[AtoPPosi];
...
void page_no_12_13_parameters()   ///// alpha to pic
{
	...
	ekka = (AtoPSelect + rand() % 9 + 1) % 10;
	dokka = (AtoPSelect + rand() % 9 + 1) % 10;
	tekka = (AtoPSelect + rand() % 9 + 1) % 10;
	chouka = rand() % 3;
	pachka = rand() % 3;
	chokka = rand() % 3;
	shatka = rand() % 3;
...
void iDraw()
{
	...
	//iShowBMP(218, 16, "lphabet_draw.bmp");
}
	...
	if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
	{
		if (page_no == 1){
			...
			if (mx >= 1035 && mx <= 1099) {
				if (((700 - my) % 100) >= 35){
					alphapicselect[anc - 'a'] = -1;
					anc = ((700 - my) / 100) * 4 + 3 + 'a';
					alphapicselect[anc - 'a'] = 0;
				}
			}*/
			if (mx >= 366 && mx <= 494 && my >= 16 && my <= 80){
				alphapicselect[anc - 'a']++;
				if (*(alphapicselect + anc - 'a') >= 0 && *(alphapicselect + anc - 'a') < *(alphapiccount + anc - 'a')) PlaySound(*(*(alphasound + anc - 'a') + *(alphapicselect + anc - 'a')), NULL, SND_FILENAME | SND_ASYNC);
			}
			if (mx >= 218 && mx <= 346 && my >= 16 && my <= 80){
				alphapicselect[anc - 'a']--;
				if (*(alphapicselect + anc - 'a') >= 0 && *(alphapicselect + anc - 'a') < *(alphapiccount + anc - 'a')) PlaySound(*(*(alphasound + anc - 'a') + *(alphapicselect + anc - 'a')), NULL, SND_FILENAME | SND_ASYNC);
			}
			...
			if (mx >= 566 && mx <= 694 && my >= 16 && my <= 80){
				bodyvariable++;
				PlaySound(bodysound[bodyvariable], NULL, SND_FILENAME | SND_ASYNC);
			}
			if (mx >= 418 && mx <= 546 && my >= 16 && my <= 80){
				bodyvariable--;
				PlaySound(bodysound[bodyvariable], NULL, SND_FILENAME | SND_ASYNC);
			}
			if (mx >= 100 && mx <= 200 && my <= 690 && my >= 640){
